...
def get_parameters(dut):
    address_width = int(dut.ADDR_WIDTH.value) // 8
    data_width = int(dut.DATA_WIDTH.value) // 8
    ram_start = int(dut.RAM_BASE_ADDRESS.value)
    ram_stop = ram_start + 2 ** int(dut.RAM_WIDTH.value)
...
def add_wstrb_mask(data_width, previous_value, write_value, wstrb):
    ...
    for i in range(data_width):
        ...
        result |= int(source) & (0xFF << (i * 8))

    return result


def compare_read_values(expected_values, read_values, burst, burst_length, address):
...
async def test_single_beat(dut, driver, address_latency, data_latency):
    ...
    expected_value = add_wstrb_mask(data_width, previous_value, write_value, strobe)
...
async def test_incr_burst(dut, size, return_rresp):
    ...
    read_values = await axim.read(
        address, len(write_values), return_rresp=return_rresp, size=size
    )
    ...
    if return_rresp:
        ...
        for i, rresp in enumerate(rresp_list):
            assert rresp is AXIxRESP.OKAY, (
                ...
                "with RRESP {} ({})".format(
                    i + 1, len(rresp_list), address, rresp.value, rresp.name
                )
            )
    ...
    expected_values = [
        ...
        for previous_value, write_value, wstrb in zip(
            previous_values, write_values, strobes
        )
    ]
...
async def test_fixed_wrap_burst(dut, size, burst_length=16):
    ...
    for burst in (AXIBurst.FIXED, AXIBurst.WRAP):
        ...
        read_values = await axim.read(address, burst_length, burst=burst, size=size)

        compare_read_values(expected_values, read_values, burst, burst_length, address)
...
async def test_narrow_burst(dut):
    ...
    compare_read_values(
        expected_values, read_values, AXIBurst.INCR, burst_length // 2, address
    )
...
async def test_unaligned(dut, size, burst):
    ...
    write_bytes, write_values = get_random_words(
        burst_length, size, size * burst_length * 8
    )

    await axim.write(address, write_values, burst=burst, size=size)
    read_values = await axim.read(unaligned_addr, burst_length, burst=burst, size=size)

    if burst is AXIBurst.FIXED:
        mask = 2 ** ((size - shift) * 8) - 1
        expected_values = [(write_values[-1] >> (shift * 8)) & mask] * burst_length
    else:
        expected_bytes = write_bytes[shift:]
        expected_words = [
            expected_bytes[i * size : (i + 1) * size] for i in range(burst_length)
        ]
        expected_values = [int.from_bytes(w, "little") for w in expected_words]

    compare_read_values(
        expected_values, read_values, burst, burst_length, unaligned_addr
    )

    # Write unaligned, read aligned
    write_bytes, write_values = get_random_words(
        burst_length, size, (size * burst_length - shift) * 8
    )
    ...
    if burst is AXIBurst.FIXED:
        last_val = first_word & mask_low | (write_values[-1] << shift * 8) & mask_high
        ...
        expected_words = [
            expected_bytes[i * size : (i + 1) * size] for i in range(burst_length - 1)
        ]
        expected_values = [int.from_bytes(w, "little") for w in expected_words]
...
async def test_unmapped(dut, driver):
    ...
    for rw in ("Read", "Write"):
        try:
            ...
            address = randrange(
                ram_stop, 2 ** (address_width * 8) - data_width, data_width
            )
        ...
        try:
            ...
            assert False, "{} at {:#x} should have failed, but did not".format(
                rw, address
            )
...
async def test_simultaneous(dut, sync, num=5):
    ...
    base_address = randrange(
        ram_start, ram_start + 4096 - 2 * num * data_width, data_width
    )
    ...
    readers = [
        cocotb.start_soon(axim.read(address, sync=sync)) for address in addresses
    ]

    dummy_addrs = [base_address + (num + i) * data_width for i in range(num)]
    dummy_writers = [
        cocotb.start_soon(axim.write(address, value, sync=sync))
    ...
    for i, (written, read) in enumerate(zip(write_values, read_values)):
        assert written == read, "#{}: wrote {:#x} but read back {:#x}".format(
            i, written, int(read)
        )
...
async def test_axi4lite_write_burst(dut):
    ...
    try:
        await axim.write(
            ram_start, [randrange(0, 2 ** (data_width * 8)) for i in range(length)]
        )
...
async def test_read_length_mismatch(dut):
    ...
    class AXI4Master_unmanaged_arlen(AXI4Master):
        _signals = [signal for signal in AXI4Master._signals if signal != "ARLEN"]
...
# ignore DeprecationWarning from TestFactory
